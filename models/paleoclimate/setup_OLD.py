...
def mountain_slope_curve(x, xsize, ysize):
    ...
    # return ysize-0.2*x # mountain_slope
    return mountain_slope
...
def glacier_surface_curve(x, xsize, ysize):
    ...
    # a =  -(xsize/ysize*0.1) * ysize/((xsize/10*8)**2)   # coefficient
    # b = xsize/10*0.8 + ysize/5                         # vertical offset
    # glacier_surface = a * x**2 + b

    # return ysize/8*7 - ysize/xsize**2 * x**2 #glacier_surface
    return glacier_surface
...
def initialize_markers(markers, materials, params, xsize, ysize):
    ...
    for j in range(0, markers.xnum):
        for i in range(0, markers.ynum):
            ...
            # glacier   # try to reproduce glacier shape from SIA
            elif markers.y[mm] >= glacier_surface_curve(markers.x[mm], xsize, ysize):
                markers.id[mm] = 2
                markers.T[mm]  = 263    # -10 °C
    ...
    # plt.title('Model setup: glacier')
    # plt.plot(markers.x, mountain_slope_curve(markers.x, xsize, ysize), label='bedrock')
    # plt.plot(markers.x, np.where(glacier_surface_curve(markers.x, xsize, ysize) >= mountain_slope_curve(markers.x, xsize, ysize), glacier_surface_curve(markers.x, xsize, ysize), np.nan), label='glacier')
    # plt.xlim(0,xsize)
    # plt.ylim(0,ysize)
    # plt.legend()
    # plt.show()
...
def initializeModel():
    ...
    # adapt params to change time
    params.t_end =   3600*365.25*24*3600           # end time
    params.ntstp_max = 1000#360                    # maximum number of timesteps
    params.tstp_max = 100*365.25*24*3600            # maximum timestep (1 year)
    ...
    # adapt params viscosity
    params.eta_min = 1e10                     # minimum viscosity
    params.eta_max = 1e20                     # maximum viscosity
    ...
    params.save_fig = 50                      # number of steps between figure output

    ###########################################################################    
    # Boundary conditions
    # pressure BCs
    P_first = np.array([0,1e5]) # [0] sets type of P BC (0=defined by one cell, 1= top and bottom), [1] sets boundary value
