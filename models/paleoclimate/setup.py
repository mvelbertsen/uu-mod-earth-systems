...
def initialize_markers(markers, materials, params, xsize, ysize):
    ...
    for j in range(0,markers.xnum):
        for i in range(0,markers.ynum):
            ...
            # bedrock
            if (markers.y[mm] <= ysize/2.-0.1*markers.x[mm]):
                markers.id[mm] = 1
                markers.T[mm]  = 273    # 0 °C
            
            # glacier   # try to reproduce glacier shape from SIA
            if (markers.y[mm] > ysize/2.-0.1*markers.x[mm] and markers.y[mm] <= ysize/8*5 - 0.1*markers.x[mm] - 2e-5*markers.x[mm]**2 + 750.): ### !!! voor nu + 750 om grotere glacier te krijgen met lagere res.
                markers.id[mm] = 2
...
def initializeModel():
    ...
    # params.t_end = 250*365.25*24*3600           # end time
    # # params.ntstp_max = 360                    # maximum number of timesteps
    ...
    # plt.scatter(markers.x, markers.y)
    # plt.show()
